...
def prompt_step1(batch):
    """
    batch: list of dicts with keys: id, post, label, category, sentiment
    """
    return (
        "You are helping with postpartum mental health data abstraction.\n"
        "For EACH item in the input list, return ONE JSON object with keys:\n"
        "  - summary (string)\n"
        "  - salient_factors (list of strings)\n"
        "  - estimated_risk (string; one of the EPDS bucket labels)\n"
        "Return a JSON LIST of objects, same length and same order as the input.\n\n"
...
def prompt_step2(step1_list):
    """
    step1_list: list of dicts as produced by step1
    """
    return (
        "You are generating synthetic postpartum diary entries.\n"
        "For EACH item in the input list, return ONE JSON object with keys:\n"
        "  - synthetic_text (120–250 word first-person diary)\n"
        "  - target (copy of the EPDS / risk bucket label)\n"
        "  - style (string, e.g., 'first-person postpartum diary')\n"
        "Return a JSON LIST of objects, same length and order as the input.\n\n"
