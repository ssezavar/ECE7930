...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--ollama-model", default="llama3:8b-instruct-q4_K_M", help="Ollama model name (if provider=ollama)")
    ...
    p.add_argument("--dedup-threshold", type=float, default=0.70, help="Jaccard overlap threshold to flag duplicates")
...
def prompt_step2(s1: Dict, target: str) -> str:
    return f"""
Write a first-person postpartum diary entry (120–280 words).
Vary the tone, style, sentence length, and narrative structure.
    ...
    for idx, row in enumerate(rows):
        ...
        try:
            ...
            if provider == "mock":
                ...
                try:
                    content = gen_text(provider, args.openai_model, args.hf_model, args.ollama_model,
                                       p1, temperature=1.2, max_tokens=260)
                    ...
                    if not content or content.strip() == "":
                        ...
                        try:
                            ...
                            # JSON extraction failed — fallback
                            s1 = mock_step1_output(post, category, label, sentiment)
                            used_mock = True
            ...
            if provider == "mock":
                ...
                try:
                    text = gen_text(provider, args.openai_model, args.hf_model, args.ollama_model, p2, temperature=1.2, max_tokens=260)
                    if not text:
